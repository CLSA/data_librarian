...
class tonometer extends base
{
  /**
   * Processes all tonometer files
   */
  public static function process_files()
  {
    $base_dir = sprintf( '%s/%s', DATA_DIR, TEMPORARY_DIR );

    // Process all TON data
    // There is only a config.tar.gz file for each participant
    output( sprintf( 'Processing tonometer files in "%s"', $base_dir ) );

    // This data only comes from the Pine Site interview
    $processed_uid_list = [];
    $file_count = 0;
    foreach( glob( sprintf( '%s/nosite/Follow-up * Site/TON/*/*', $base_dir ) ) as $filename )
    {
      $re = '#nosite/Follow-up ([0-9]) Site/TON/([^/]+)/config.tar.gz$#';
      $matches = [];
